...
'''
This script runs a calibration profile for the robot provided by -Thesis-. 

First the left actuator is held at 3 different positions - 0, 50, 100 mm actuation. At each
left actuator position, the right actuator is run from 0-100mm in 2mm increments 5 times. These runs are averaged
and the standard deviation of the data is calculated. This is then repeated for the Left actuator with the right
actuator being held in identical position.

Once this data is collected, use calibration_visualization.py to generate graphs that display this data. 

---Note: the actuators are controlled up via a grbl CNC controller so often x and y are used to represent actuators
	X Axis = Right Actuator
	Y Axis = Left Actuator
...
if __name__ == '__main__':
	...
	#collect and average the readings and calculate standard deviations
	...
	#save to files... json
	run = 0
	baseDir = 'logs/calibration_files/' + robotName + '/Calibration_run_'
